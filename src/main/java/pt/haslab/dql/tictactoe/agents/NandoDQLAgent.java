 ...
 */
public class NandoDQLAgent extends Agent {
    ...
    private void initNN(){
        if(!loadNNFromFile()) {
            //build NN with 9-neuron input layer, 27-neuron hidden layer, 9-neuron hidden layer, 9-neuron output layer
            network = new BasicNetwork();
            network.addLayer(new BasicLayer(null, true, 9));
            network.addLayer(new BasicLayer(new ActivationSigmoid(), true, 27));
