...
class NandoDQLAgent(nn.Module):
    ...
    def __init__(self, state_size: int, move_size: int, config: QLearningConfig):
        ...
        print(f'Model initialized with state size: {state_size}, move size: {move_size}')
    ...
    def remember(self, state: np.ndarray, move: str, reward: float, next_state: np.ndarray, done: bool) -> None:
    ...
    def replay(self) -> None:
        ...
        minibatch = random.sample(self.memory, min(len(self.memory), self.q_learn_config.batch_size))
        ...
        for memory in minibatch:
            ...
            if not memory.done:
                ...
                with torch.no_grad():
                    target += self.q_learn_config.gamma * torch.max(self.target_model(next_state_tensor)[0])
            ...
            loss = self.loss_fn(self.model(torch.FloatTensor(memory.state).unsqueeze(0)), target_f)
            loss.backward()
            self.optimizer.step()
    ...
    def train(self, adversary: Agent, output_file: str) -> List[float]:
        ...
        for e in range(self.q_learn_config.epochs):
            ...
            print(f"'X': {agents[0].__class__.__name__}, 'O': {agents[1].__class__.__name__}")
            ...
            while game.current_state == GameState.PLAYING:
                ...
                if current_agent == self:
                    self.remember(board_state.to_numpy_arr(), move, reward, next_state.to_numpy_arr(), done)
                
                board_state = next_state
                
                if done: # Check if the game is over
                    print(f'Epoch: {e + 1}/{self.q_learn_config.epochs}, Reward: {reward}, e: {self.q_learn_config.epsilon:.2}\n')
                    total_reward += reward
        ...
        print(f'Training complete (Total reward: {total_reward}). Saving model to {output_file}')
